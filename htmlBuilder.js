...
module.exports = (client, ticketId, channelName, msgs) => `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
${msgs.map((msg) => {
      ...
      <div class=chatlog__message-group>
        <div class=chatlog__author-avatar-container><img class=chatlog__author-avatar src=${msg.author.displayAvatarURL({ dynamic:true })}></div>
        <div class=chatlog__messages>
          ...
          <div class=chatlog__message id=message-${msg.id} data-message-id=${msg.id}>
            ...
            ${getContent(client, msg, msg.content)}
              ${msg.attachments.size ? msg.attachments.map(attachment => {
		return `<div class=chatlog__attachment><a href=${attachment.url}><img class=chatlog__attachment-thumbnail src=${attachment.url}></a></div>`;
              ...
              ${msg.reactions.cache.size ? `<div class=chatlog__reactions>
              <div class=chatlog__reactions>
              ${ msg.reactions.cache.map(reaction => {
		return `<div class=chatlog__reaction><img class=emoji--small alt=${reaction.emoji.name} title=${reaction.emoji.name} src="${reaction.emoji.url ? reaction.emoji.url : `https://twemoji.maxcdn.com/2/72x72/${emojiUnicode(reaction.emoji.name)}.png`}"><span class=chatlog__reaction-count>1</span></div>`;
	}).join('\n') }
              </div>
            </div>
            </div>` : ''}
...
function getContent(client, msg, content) {
	...
	for (const text of contentArray) {
		if (['<@&', '<@', '<#'].some(t => text.startsWith(t)) && text.endsWith('>')) newContent.push(getMention(client, msg, text));
		else newContent.push(text);
	}
	return newContent.join(' ');
}

function getMention(client, msg, text) {
	...
	switch (type) {
	case 'user': return `<span class=mention title=${client.users.cache.get(id)?.tag || id}>@${client.users.cache.get(id)?.username || id}</span>`;
	case 'channel': return `<span class=mention title=${msg.guild.channels.cache.get(id)?.name || id}>#${msg.guild.channels.cache.get(id)?.name || id}</span>`;
	case 'role': return `<span class=mention title=${msg.guild.roles.cache.get(id)?.name || id}>@${msg.guild.roles.cache.get(id)?.name || id}</span>`;
	}
}

function getEmbed(client, msg, embed) {
  ...
  <div class=chatlog__embed-content-container>
    <div class=chatlog__embed-content>
      <div class=chatlog__embed-text>
        <div class=chatlog__embed-description><span class="markdown">${getContent(client, msg, embed.description)}</span>
        ${embed.image ? `<div class=chatlog__embed-image-container><div class=chatlog__embed-image>${embed.image.url}</div></div>` : ''}
        </div>
        ${embed.fields.length ? `<div class=chatlog__embed-fields>${embed.fields.map(field => `<div class=chatlog__embed-field><div class=chatlog__embed-field-name>${field.name}</div><div class="chatlog__embed-field-value">${getContent(client, msg, field.value)}${field.inline ? '<div class=chatlog__embed-field>--inline></div>' : ''}</div></div>
        </div>`)}</div>` : ''}
      </div>
      ${embed.thumbnail ? `<div class=chatlog__embed-thumbnail-container><a class=chatlog__embed-thumbnail-link><img class=chatlog__embed-thumbnail src="${embed.thumbnail.url}"></a></div>` : '' }
     </div>
    ${embed.footer ? `<div class=chatlog__embed-footer>${embed.footer.iconURL ? `<img class=chatlog__embed-footer-icon src=${embed.footer.iconURL}></img>` : ''}<span class=chatlog__embed-footer-text>${embed.footer.text}</span></div>` : '' }
  </div>
</div>`;
}
...
function filterContent(content) {
	const backtick = 0xdeadbeef * '`'.codePointAt(),
		asterik = 0xdeadbeef * '*'.codePointAt(),
		tilde = 0xdeadbeef * '~'.codePointAt(),
		underscore = 0xdeadbeef * '_'.codePointAt(),
		pipe = 0xdeadbeef * '|'.codePointAt(),
		sign = 0xdeadbeef * '>'.codePointAt(),
		square_bracket_left = 0xdeadbeef * '['.codePointAt(),
		square_bracket_right = 0xdeadbeef * ')'.codePointAt(),
		bracket_left = 0xdeadbeef * '('.codePointAt(),
		bracket_right = 0xdeadbeef * ')'.codePointAt();
	...
	const codes = {
		...
		'\\)': bracket_right.toString().replace(),
	};
	...
	filteredContent = content.replace(/(\\)(`|\*|~|_|\||\[|\]|\(|\))/gm, value => codes[value]);

	// Asterik
	while (filteredContent.match(/\*\*((\n)*((\n)*.+?(\n)*)+?(\n)*)+?\*\*/gm)) {
		filteredContent = filteredContent.replace(/\*\*((\n)*((\n)*.+?(\n)*)+?(\n)*)+?\*\*/gm, value => `<strong>${value.slice(2, -2)}</strong>`);
		filteredContent = filteredContent.replace(/\*((\n)*((\n)*.+?(\n)*)+?(\n)*)+?\*/gm, value => `<em>${value.slice(1, -1)}</em>`);
	}

	// Underscore
	while (filteredContent.match(/__((\n)*((\n)*.+?(\n)*)+?(\n)*)+?__|_((\n)*((\n)*.+?(\n)*)+?(\n)*)+?_/gm)) {
		filteredContent = filteredContent.replace(/__((\n)*((\n)*.+?(\n)*)+?(\n)*)+?__/gm, value => `<u>${value.slice(2, -2)}</u>`);
		filteredContent = filteredContent.replace(/_((\n)*((\n)*.+?(\n)*)+?(\n)*)+?_/gm, value => `<em>${value.slice(2, -2)}</em>`);
	}


	// Backtick
	while (filteredContent.match(/```((\n)*((\n)*.+?(\n)*)+?(\n)*)+?```|``((\n)*((\n)*.+?(\n)*)+?(\n)*)+?``|`((\n)*((\n)*.+?(\n)*)+?(\n)*)+?`/gm)) {
		filteredContent = filteredContent.replace(/```((\n)*((\n)*.+?(\n)*)+?(\n)*)+?```/gm, value => `<code class="block">${value.slice(3, -3)}</code>`);
		filteredContent = filteredContent.replace(/``((\n)*((\n)*.+?(\n)*)+?(\n)*)+?``/gm, value => `<code class="inline">${value.slice(2, -2)}</code>`);
		filteredContent = filteredContent.replace(/`((\n)*((\n)*.+?(\n)*)+?(\n)*)+?`/gm, value => `<code class="inline">${value.slice(1, -1)}</code>`);
	}

	// Quotes
	if (filteredContent.match(/^>\s/m)) {
		filteredContent = filteredContent.replace(/^> .+[?\n]/mg, value => `<span class="quote">${value.slice(2)}</span>`);
	}
	if (filteredContent.match(/((^>> .+)(.+|\n.+)*)/gm)) {
		filteredContent = filteredContent.replace(/((^>> .+)(.+|\n.+)*)/mg, value => `<div class="quote">${value.slice(3)}</div>`);
	}

	// Tilde
	while (filteredContent.match(/~~((\n)*((\n)*.+?(\n)*)+?(\n)*)+?~~/gm)) {
		filteredContent = filteredContent.replace(/~~((\n)*((\n)*.+?(\n)*)+?(\n)*)+?~~/gm, value => `<s>${value.slice(2, -2)}</s>`);
	}

	// Pipe
	while (filteredContent.match(/\|\|((\n)*((\n)*.+?(\n)*)+?(\n)*)+?\|\|/gm)) {
		filteredContent = filteredContent.replace(/\|\|((\n)*((\n)*.+?(\n)*)+?(\n)*)+?\|\|/gm, value => `<span id="spoiler" onClick="unHideSpoiler()" class="spoiler">${value.slice(2, -2)}</span>`);
	}
	...
	const regex = new RegExp(`${backtick}|${asterik}|${tilde}|${underscore}|${pipe}|${sign}|${square_bracket_left}|${square_bracket_right}|${bracket_left}|${bracket_right}`, 'g');
	filteredContent = filteredContent.replace(regex, value => reverseCodes[value]);
